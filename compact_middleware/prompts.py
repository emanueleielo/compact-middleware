...
Direct port of Claude Code's 9-section structured prompts from
``src/services/compact/prompt.ts``, adapted for Python string formatting.

Includes:
- ``NO_TOOLS_PREAMBLE`` / ``NO_TOOLS_TRAILER``: prevent tool calls during summarization
- ``BASE_COMPACT_PROMPT``: full 9-section summarization prompt
- ``PARTIAL_COMPACT_PROMPT``: for partial compaction (recent messages only)
- ``PARTIAL_COMPACT_UP_TO_PROMPT``: for partial compaction (prefix only)
- ``format_compact_summary()``: strips ``<analysis>`` scratchpad
- ``get_compact_user_summary_message()``: builds the continuation message
"""
...
# ---------------------------------------------------------------------------
# Analysis instructions (scratchpad)
# ---------------------------------------------------------------------------
...
def get_compact_prompt(custom_instructions: str | None = None) -> str:
    """Build the full compaction prompt with no-tools enforcement.
...
def get_partial_compact_prompt(
    ...
    """Build a partial compaction prompt.

    Args:
        custom_instructions: Optional additional instructions.
        direction: ``'from'`` = summarize recent (after retained prefix).
                   ``'up_to'`` = summarize prefix (before retained recent).
...
def format_compact_summary(summary: str) -> str:
    """Strip the ``<analysis>`` scratchpad and format ``<summary>`` tags.

    The ``<analysis>`` block improves summary quality during generation but
    has no informational value once the summary is written.

    Args:
        summary: Raw summary from the LLM, potentially containing XML tags.

    Returns:
        Cleaned summary with analysis stripped and summary tags replaced.
    """
    formatted = summary
...
def get_compact_user_summary_message(
    ...
    *,
    ...
    This is the ``HumanMessage`` content that replaces the compacted history.

    Args:
        summary: The raw summary (will be formatted).
        suppress_follow_up_questions: If True, instruct model to continue without asking.
        transcript_path: Path to the full transcript on the backend.
        recent_messages_preserved: Whether recent messages are kept verbatim.

    Returns:
        Formatted continuation message string.
    """
